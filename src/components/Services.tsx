...
const Services = () => {
  ...
  return (
    <section id="services" className="py-20 bg-tp-bg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-16">
          <h2 className="text-4xl md:text-5xl font-bold text-tp-text mb-6">Our Services</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <div key={index} className="bg-tp-light p-8 rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 group">
              <div className="bg-tp-accent w-16 h-16 rounded-lg flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300">
                <service.icon className="text-white" size={32} />
              </div>

              <h3 className="text-xl font-bold text-tp-text mb-4">{service.title}</h3>
              <p className="text-tp-muted mb-6 leading-relaxed">{service.description}</p>

              <ul className="space-y-2 mb-6">
                {service.features.map((feature, featureIndex) => (
                  <li key={featureIndex} className="text-tp-muted flex items-center">
                    <div className="w-2 h-2 bg-tp-accent rounded-full mr-3"></div>
                    {feature}
                  </li>
                ))}
              </ul>
              ...
              <div className="flex items-center justify-between">
                <button 
                  ...
                  className="text-tp-btn hover:text-tp-btn-hover font-semibold transition-colors duration-200"
                >
                  Learn More →
                </button>
              </div>
        ...
        <div className="text-center mt-16">
          <div className="bg-tp-accent text-white p-8 rounded-2xl">
            ...
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
              <button 
                onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
                className="bg-white text-tp-btn hover:bg-tp-light px-6 py-3 rounded-lg font-semibold transition-colors duration-200"
              >
